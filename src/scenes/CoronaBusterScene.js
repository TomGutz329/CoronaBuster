...
export default class CoronaBusterScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('enemies', 'images/ememy.png');
  ...
  create() {
    ...
    this.time.addEvent({
      delay: Phaser.Math.Between(100, 500), //--------> Delay random range 1-5 seconds
    ...
    this.lifeLabel = this.add.text(10, 28, "Life", {
      fontSize: "16px",
      fill: "black",
      backgroundColor: "white",
    }).setDepth(1);
    ...
    this.time.addEvent({
      delay: 20000, // handsanitizer spawn every 20s
  ...
  createButton(){
    ...
    let nav_right = this.add.image(nav_left.x + nav_left.displayWidth, 550,'right-btn').setInteractive().setDepth(0.5).setAlpha(0.8)
  ...
  spawnEnemy() {
    const config = {
       speed: 80,       //-----------> Set the speed and rotation size of the enemy
       rotation: 0
    };
    // @ts-ignore
